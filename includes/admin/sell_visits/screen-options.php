...
require_once AFFILIATEWP_PLUGIN_DIR . 'includes/admin/sell_visits/class-list-table.php';
 ...
 * @since 1.7
 */
function affwp_sell_visits_screen_options() {

	$screen = affwp_get_current_screen();

	if ( $screen !== 'affiliate-wp-sell-visits' ) {
		return;
	}
...
add_action( 'load-affiliates_page_affiliate-wp-sell_visits', 'affwp_sell_visits_screen_options' );
 ...
 * @return mixed
 */
function affwp_sell_visits_set_screen_option( $status, $option, $value ) {
...
add_filter( 'set-screen-option', 'affwp_sell_visits_set_screen_option', 10, 3 );
