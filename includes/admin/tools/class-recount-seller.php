...
class Affiliate_WP_Recount_Seller {
	...
	public function __construct() {
		add_action( 'affwp_recount_seller_stats', array( $this, 'process_recount' ) );
	}
	...
	public function recount_referrals( $affiliate_id = 0 ) {
		...
		$earnings = affiliate_wp()->referrals->count( array( 'affiliate_id' => $affiliate_id, 'status' => ['paid', 'refunded'], 'sell' => true ) );
		affiliate_wp()->affiliates->update( $affiliate_id, array( 'sell_referrals' => $earnings ), '', 'affiliate' );
	...
	public function recount_visits( $affiliate_id = 0  ) {
		...
		$earnings = affiliate_wp()->visits->count( array( 'affiliate_id' => $affiliate_id, 'sell' => true ) );
		affiliate_wp()->affiliates->update( $affiliate_id, array( 'sell_visits' => $earnings ), '', 'affiliate' );
