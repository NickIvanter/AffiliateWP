<?php
namespace AffWP\Sell_Visit\Admin\Reports;
...
class Tab extends Reports\Tab {
	...
	public function __construct() {
		$this->tab_id   = 'sell-visits';
		$this->label    = __( 'Sell Visits', 'affiliate-wp' );
		$this->priority = 0;
	...
	public function register_tiles() {
		$this->register_tile( 'total_sell_visits', array(
			'label'           => __( 'Total Visits', 'affiliate-wp' ),
			'type'            => 'number',
			'data'            => affiliate_wp()->visits->count( ['sell' => true] ),
			'comparison_data' => __( 'All Time', 'affiliate-wp' ),
		) );

		$this->register_tile( 'total_sell_visits_date', array(
			'label'           => __( 'Total Visits', 'affiliate-wp' ),
			'type'            => 'number',
			'context'         => 'secondary',
		...
		$this->register_tile( 'converted_sell_visits', array(
		...
		$this->register_tile( 'conversion_sell_rate', array(
	...
	public function top_referrer_tile() {
		...
		if ( $referrers ) {
			...
			$this->register_tile( 'top_sell_referrer', array(
